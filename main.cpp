...
int main()
{
    ...
    for (i = 0; i < nthreads; ++i) {
        sum_threads.push_back(thread(single_sum_thread, i, nthreads, sum));
    }
    // join the threads with the main thread
    for (i = 0; i < nthreads; ++i) {
        sum_threads.at(i).join();
    }
    ...
    // }
    vector<thread> sum_square_threads;

    for (i = 0; i < nthreads; ++i) {
        sum_square_threads.push_back(thread(pi_sum_thread, i, sum));
    }

    for (i = 0; i < nthreads; ++i) {
        sum_square_threads.at(i).join();
    }
